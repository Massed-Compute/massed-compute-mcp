...
`scripts/smoke-test.mjs` runs nightly, unauthenticated, and asserts two things about the hosted endpoint: the public `GET` server card still carries a protocol version and a non-empty tool catalog, and an unauthenticated JSON-RPC `POST` is still rejected with 401 carrying a `WWW-Authenticate: Bearer ... resource_metadata=` challenge. That second check is not paranoia — anonymous `POST` was closed deliberately (MCDEV-482) so that MCP clients see the challenge and start OAuth discovery; if it ever answers 200 again, or answers a bare 401 with the header dropped, the OAuth flow has quietly stopped being reachable. The script fails open (warn, exit 0) only on failures that are plausibly transient — a connection error, a timeout, or a 5xx/408/429. A permanent 4xx or a non-JSON body fails closed with exit 2, because those are contract failures, and skipping them would leave the nightly green forever while both checks stopped running. A 403 stays on the fail-closed side — an edge mitigation against the runner IP can produce one, so the failure message names the WAF and prints the response content-type, but a permanently closed route is precisely what this job exists to surface. Adding or changing tools happens in the service that backs the hosted endpoint, not in this repo.
